...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
            ...
            test_data=None, predict_data=None):
        ...
        while 1:
            ...
            arc = self.evaluate(test_data)
            # print "Epoch {0}: {1} / {2}".format(count, arc, n_test)
            if (float(arc)/n_test > 0.90) or count > 9999:              # 迭代停止条件：准确率高于90%或迭代次数高于9999
                print("Epoch {0}: {1} / {2}".format(count, arc, n_test))
                result = np.argmax(self.feedforward(predict_data))
                print("Predict Result of 2020: {0}".format(result))
                break
